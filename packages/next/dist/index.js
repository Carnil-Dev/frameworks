...
function createCarnilHandler(config) {
  ...
  return async function carnilHandler(req) {
    try {
      ...
      if (!customerId) {
        return server.NextResponse.json({ error: "Customer ID is required" }, { status: 401 });
      }
      ...
      switch (action) {
        ...
        case "getAIUsageMetrics":
          ...
          break;
        default:
          return server.NextResponse.json({ error: `Unknown action: ${action}` }, { status: 400 });
      }
...
function createCarnilWebhookHandler(config) {
  ...
  return async function carnilWebhookHandler(req) {
    try {
      ...
      if (!webhookSecret) {
        return server.NextResponse.json({ error: "Webhook secret not configured" }, { status: 400 });
      }
      const isValid = await carnil.verifyWebhook(body, signature, webhookSecret);
      if (!isValid) {
        return server.NextResponse.json({ error: "Invalid webhook signature" }, { status: 400 });
      }
...
exports.createCarnilHandler = createCarnilHandler;
exports.createCarnilServerActions = createCarnilServerActions;
exports.createCarnilWebhookHandler = createCarnilWebhookHandler;
